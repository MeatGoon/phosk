...
=======
/*
 * public class QRTableQuery { static {
 * 
 ...
 * DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","jun","jun"
 * ); statement = connection.createStatement(); resultSet =
 * statement.executeQuery("select*from phosk_QRTable");
 * 
 * while(resultSet.next()) { String branch_num =
 * resultSet.getString("branch_num"); int QRTable_Num =
 * resultSet.getInt("QRTable_Num"); String QRTable_URL =
 * resultSet.getString("QRTable_URL");
 * 
 * System.out.println("지점번호:" + branch_num + ",테이블번호:" + QRTable_Num + ",테이블주소"
 * + QRTable_URL); } } catch(Exception e) { e.printStackTrace(); } finally { try
 * { if(resultSet !=null) resultSet.close(); if(statement !=null)
